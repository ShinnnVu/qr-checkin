...
import { Box, Center, Divider, Flex, Heading, HStack, Input, Pressable, Text, View, VStack } from "native-base";
...
const CheckinHistory = ({ route, navigation }: { route: any; navigation: any }) => {
    ...
    const filter = (month: number) => {
        const pages = chunk(data.filter(d => d.date.getMonth() === month), 7).map((page, i) => {
            const p = page.map((h, j) => (
                <Flex key={h.date.toDateString() + j} direction="row" justify={"center"} my={5}>
                    <Text flex={1} textAlign={"center"}>{getDateFormat(h.date)}</Text>
                    <Text flex={1} textAlign={"center"}>{h.checkin.toLocaleTimeString()}</Text>
                    <Text flex={1} textAlign={"center"}>{h.checkout.toLocaleTimeString()}</Text>
                </Flex>
            ))
            return (
                <View key={i + 1}>
        ...
        if (pages.length === 0) {
            pages.push(<View flex={1} key={1} justifyContent={"center"}><Text>No Data</Text></View>);
        } 
        setHistory(pages);
        setCurrentPage(0);
    };
    ...
    return (
        <Box flex={1} py={3} safeArea>
            <VStack flex={1} >
                <HeaderTwo title="Test" back={() => navigation.navigate("Home")} />
                ...
                <Flex direction="row" justify={"center"} my={5}>
                    <Text flex={1} textAlign={"center"} bold={true}>Date</Text>
                    <Text flex={1} textAlign={"center"} bold={true}>Checkin</Text>
                    <Text flex={1} textAlign={"center"} bold={true}>Checkout</Text>
                </Flex>
                <Divider />
            ...
            <HStack alignSelf={"center"} alignItems={"center"} mb={5}>
                ...
                <Icon_Button
                    onPress={() => { if (currentPage < history.length - 1) { setCurrentPage(currentPage + 1); pager?.current?.setPage(currentPage + 1); }}}
                    icon={RIGHT_CAVRET}
                    pColor={color.GRAY_BUTTON_CLICK}
                    upColor={color.GRAY_BUTTON}
                />
